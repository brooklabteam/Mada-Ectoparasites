...
afa <- dplyr::select(afa, roost_site, sampleid, bat_species, bat_sex, bat_age_class, processing_date, mass_forearm_residual, Male, Female)

head(afa)
afa <- afa[complete.cases(afa),]

#and melt the data
afa.M <- dplyr::select(afa, names(afa)[1:8])
afa.F <- dplyr::select(afa, c(names(afa)[1:7], "Female"))
...
afa$month <- as.factor(as.numeric(month(afa$processing_date)))
unique(afa$roost_site)
afa$roost_site[afa$roost_site=="Angavobe_Edup" | afa$roost_site=="Angavokely_Edup" | afa$roost_site =="Maromizaha_Rmad"] <- "central-east"
afa$roost_site[afa$roost_site!="central-east"] <- "north"


# rousettus 
# north 
t.test(x=afa$count[afa$fly_sex=="female" & afa$bat_species=="Rousettus madagascariensis" & afa$roost_site=="north"], y=afa$count[afa$fly_sex=="male"& afa$bat_species=="Rousettus madagascariensis" & afa$roost_site=="north"])
# p-value = 4.644e-05
#mean of x mean of y 
# 1.901099  3.109890 

#central-east 
t.test(x=afa$count[afa$fly_sex=="female" & afa$bat_species=="Rousettus madagascariensis" & afa$roost_site=="central-east"], y=afa$count[afa$fly_sex=="male"& afa$bat_species=="Rousettus madagascariensis" & afa$roost_site=="central-east"])
# p-value = 0.07198
#mean of x mean of y 
# 3.512563  4.027638 
...
#eidolon
#north
t.test(x=afa$count[afa$fly_sex=="female" & afa$bat_species=="Eidolon dupreanum"& afa$roost_site=="north"], y=afa$count[afa$fly_sex=="male"& afa$bat_species=="Eidolon dupreanum"& afa$roost_site=="north"])
# p-value = 2.962e-06
#mean of x mean of y 
# 1.4250    2.8625 

#central-east
t.test(x=afa$count[afa$fly_sex=="female" & afa$bat_species=="Eidolon dupreanum"& afa$roost_site=="central-east"], y=afa$count[afa$fly_sex=="male"& afa$bat_species=="Eidolon dupreanum"& afa$roost_site=="central-east"])
# p-value = 0.1496
#mean of x mean of y 
# 1.892405  2.240506 
...
ggplot(afa, aes(x=month, y=count)) + theme_bw() + facet_grid(bat_species~roost_site, scales = "free", space = "free") +
  geom_violin(aes(fill=fly_sex), draw_quantiles = c(0,.25,.5,.75))  + 
  scale_fill_manual(values=c("hotpink2", "blue"), name = "bat fly sex") +
  scale_color_manual(values=c("hotpink2", "blue"), name = "bat fly sex") +
  scale_x_discrete(breaks=c(1:12), labels=   c("jan","feb", "mar","apr",  "may","jun",  "jul","aug", "sep","oct", "nov", "dec"))+
  geom_point(aes(color=fly_sex), position = position_jitterdodge(), size=.2, show.legend = F) +
  theme(panel.grid = element_blank(), legend.position = "inside", legend.position.inside = c(.85,.85), axis.title.x = element_blank()) 

label.dat <- cbind.data.frame(bat_species=c("Eidolon dupreanum", "Eidolon dupreanum", "Rousettus madagascariensis","Rousettus madagascariensis"), roost_site = c("central-east", "north","central-east", "north"))
label.dat$label = c("p=0.15", "p<0.001***", "p=0.07", "p<0.001***")

FigS3 <- ggplot(afa, aes(x=fly_sex, y=count)) + theme_bw() + facet_grid(bat_species~roost_site) +
  geom_violin(aes(fill=fly_sex), draw_quantiles = c(0,.25,.5,.75), show.legend = F)  + 
  scale_fill_manual(values=c("hotpink2", "blue")) +
  #scale_x_discrete(breaks=c(1:12), labels=   c("jan","feb", "mar","apr",  "may","jun",  "jul","aug", "sep","oct", "nov", "dec"))+
  ...
  theme(panel.grid = element_blank(), #legend.position = "inside", legend.position.inside = c(.1,.75), 
        ...
        strip.text.x = element_text(size=16)) + xlab("bat fly sex") +
  guides(color=guide_legend(title = "month of\nsampling"));FigS3
